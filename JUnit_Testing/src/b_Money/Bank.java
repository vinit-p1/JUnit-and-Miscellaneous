...
//	statement get(accountid) never opens new account it just gets the account we need to add new account in the HashTable so we need to use "put" method
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {  // Got a null point exception because of missing "!" Everytime it checks if account exists it throws exception
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			account.withdraw(money); // Found addition in the test case over here deposit method was used instead of withdraw..
		}
	}
